...
const AdmParameterPage = () => {
    ...
    const [admParameterService,] = useState<AdmParameterService>(new AdmParameterService());
    ...
    }, [admParameterCategoryService, admParameterService, cols]);
    ...
    const exportPdf = () => {
        ...
        listaAdmParameter.forEach(item => {
            if (item.admParameterCategory){ 
                data.push([item.id, item.admParameterCategory.description, item.code, item.value, item.description])
            }
        });
