<template>
    <ul id="treeviewUL">
        <li v-for="(item, index) in nodeLista">
            <span class="caret" @click="caretOnClick($event, item)">{{ item.label }}</span>
            <ul class="nested">
                <li v-for="(subitem, subindex) in item.children" style="margin-top: 10px; margin-left: 20px;">
                    <span class="folha" @click="folhaOnClick($event, subitem)">{{ subitem.label }}</span>
                </li>
            </ul>
        </li>
    </ul>
        ...
        onMounted(() => {
            ...
            if (caret){
                for (k = 0; k < caret.length; k++) {
                    ...
                    caret[k].classList.toggle("caret-down");
                }
            }
        });
        */
